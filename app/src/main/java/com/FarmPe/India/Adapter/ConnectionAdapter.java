...
holder.whatsapp_call.setOnClickListener(new View.OnClickListener() {
    ...
    {
        getContactIDFromNumber("8861948875",activity);
        String rowContactId = null;
        boolean hasWhatsApp;

        String[] projection = new String[]{ContactsContract.RawContacts._ID};
        String selection = ContactsContract.Data.CONTACT_ID + " = ? AND account_type IN (?)";
        String[] selectionArgs = new String[]{rowContactId, "com.whatsapp"};
        Cursor cursor = activity.getContentResolver().query(ContactsContract.RawContacts.CONTENT_URI, projection, selection, selectionArgs, null);
        if (cursor != null) {
